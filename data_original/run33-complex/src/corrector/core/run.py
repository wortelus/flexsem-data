...
def run(cfg: Settings, sem_cfg: FlexSEMSettings):
    ...
    # Define parameters for each trajectory type
    # Phase 1: Single-axis characterization
    SAWTOOTH_RANGE = Distance.from_micrometers(6.0)
    SAWTOOTH_STEP_FAST = Distance.from_nanometers(300)
    SAWTOOTH_STEP_SLOW = Distance.from_nanometers(150)
    SAWTOOTH_REPS = 2

    # Phase 2: Multi-axis characterization
    SQUARE_SIDE = Distance.from_micrometers(3)
    SQUARE_POINTS_PER_SIDE = 30
    CIRCLE_RADIUS = Distance.from_micrometers(2.5)
    CIRCLE_POINTS = 80

    # Phase 3: Generalization
    RANDOM_WALK_STEPS = 100
    RANDOM_WALK_MAX_STEP = Distance.from_micrometers(5)
    ...
    try:
        ...
        for center_x in grid_centers_x:
            for center_y in grid_centers_y:
                ...
                # --- PHASE 1: Sawtooth (Fast) ---
                traj = generate_sawtooth_trajectory('x', base_position, SAWTOOTH_RANGE, SAWTOOTH_STEP_FAST,
                                                    SAWTOOTH_REPS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position,
                                                        "sawtooth_x_fast", iteration_counter)
                all_experiments_data.extend(results)

                traj = generate_sawtooth_trajectory('y', base_position, SAWTOOTH_RANGE, SAWTOOTH_STEP_FAST,
                                                    SAWTOOTH_REPS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position,
                                                        "sawtooth_y_fast", iteration_counter)
                all_experiments_data.extend(results)

                # --- PHASE 1: Sawtooth (Slow) ---
                traj = generate_sawtooth_trajectory('y', base_position, SAWTOOTH_RANGE, SAWTOOTH_STEP_SLOW,
                                                    SAWTOOTH_REPS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position,
                                                        "sawtooth_y_slow", iteration_counter)
                all_experiments_data.extend(results)

                traj = generate_sawtooth_trajectory('x', base_position, SAWTOOTH_RANGE, SAWTOOTH_STEP_SLOW,
                                                    SAWTOOTH_REPS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position,
                                                        "sawtooth_x_slow", iteration_counter)
                all_experiments_data.extend(results)

                # --- PHASE 2: Square ---
                traj = generate_square_trajectory(base_position, SQUARE_SIDE, SQUARE_POINTS_PER_SIDE)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position,
                                                        "square",
                                                        iteration_counter)
                all_experiments_data.extend(results)
                ...
                # --- PHASE 2: Circle ---
                traj = generate_circle_trajectory(base_position, CIRCLE_RADIUS, CIRCLE_POINTS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position, "circle",
                                                        iteration_counter)
                all_experiments_data.extend(results)

                # --- PHASE 3: Random Walk ---
                traj = generate_random_walk_2d(base_position, RANDOM_WALK_STEPS, RANDOM_WALK_MAX_STEP)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, traj, base_position,
