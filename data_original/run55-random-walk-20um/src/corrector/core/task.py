...
class ExposureTask(BaseTask):
    ...
    def execute(self, ref_img_path: Path = None) -> (float, float, Path):
        ...
        return (Distance.from_nanometers(0), Distance.from_nanometers(0), 0), None, None  # No offset change during exposure
...
class CorrectionTask(BaseTask):
    ...
    def execute(self, ref_img_path: Path = None) -> (str, str, Path):
        ...
        # Magnify -> Focus -> Scan
        img_path, metadata_path = correction_scan(self.cfg, self.cmd_factory, self.manager, self.step, settle_time_s=3)

        # Calculate drift
        metadata = load_metadata(metadata_path)
        (dx, dy), conf = calculate_drift(self.cfg, ref_img_path, img_path, metadata)
...
class UpdateReferenceTask(BaseTask):
    ...
    def execute(self, ref_img_path: Path = None) -> (str, str, Path):
        ...
        # Magnify -> Focus -> Scan
        img_path, metadata_path = correction_scan(self.cfg, self.cmd_factory, self.manager, self.step, settle_time_s=5)

        return (Distance.from_nanometers(0), Distance.from_nanometers(0), 1.0), img_path, metadata_path  # No offset change during reference update
