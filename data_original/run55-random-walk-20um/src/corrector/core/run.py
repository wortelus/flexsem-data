...
def generate_major_minor_loop_trajectory(axis: str,
                                         start_pos: dict,
                                         distance_step_list: list[tuple[Distance, Distance]],
                                         offset_first_round: Distance) -> list:
    ...
    for i, (major_step, minor_step) in enumerate(distance_step_list):
        ...
        if offset_first_round != Distance.from_nanometers(0) and i == 0:
            forward_nanometers = major_step.nanometers + offset_first_round.nanometers
        else:
            forward_nanometers = major_step.nanometers

        while pos.nanometers < forward_nanometers:
            pos += minor_step
            if axis == 'x':
...
def run(cfg: Settings, sem_cfg: FlexSEMSettings):
    ...
    #     (Distance.from_nanometers(2000), Distance.from_nanometers(50)),
    #     (Distance.from_nanometers(750), Distance.from_nanometers(50)),
    #     (Distance.from_nanometers(2400), Distance.from_nanometers(400)),
    #     (Distance.from_nanometers(3000), Distance.from_nanometers(300)),
    #     (Distance.from_nanometers(1800), Distance.from_nanometers(300)),
    #     (Distance.from_nanometers(4000), Distance.from_nanometers(400)),
    #     (Distance.from_nanometers(3600), Distance.from_nanometers(400)),
    #     (Distance.from_nanometers(1800), Distance.from_nanometers(200)),
    #     (Distance.from_nanometers(2700), Distance.from_nanometers(300)),
    #     (Distance.from_nanometers(1000), Distance.from_nanometers(50)),
    #     (Distance.from_nanometers(1600), Distance.from_nanometers(100)),
    #     (Distance.from_nanometers(3300), Distance.from_nanometers(300)),
    #     (Distance.from_nanometers(2000), Distance.from_nanometers(400)),
    #     (Distance.from_nanometers(1200), Distance.from_nanometers(100)),
    #     (Distance.from_nanometers(2200), Distance.from_nanometers(100)),
    #     (Distance.from_nanometers(500), Distance.from_nanometers(50)),
    # ]
    #
        ...
        for center_x in grid_centers_x:
            for center_y in grid_centers_y:
                ...
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
                                                        "random_walk_2026-02-10", iteration_counter)
                all_experiments_data.extend(results)

                # traj = generate_major_minor_loop_trajectory("x", base_position, MAJOR_MINOR_LOOP_STEPS, Distance.from_nanometers(1150))
                # lets print .tolist() sequence of x nanometer steps
                # print(f"u_target = [{', '.join(str(x.nanometers) for x, y in traj)}]")
                # wait for user input
                # input("Press Enter to continue...")
