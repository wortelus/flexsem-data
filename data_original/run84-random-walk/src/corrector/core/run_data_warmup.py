...
from corrector.utils.sem import send_command_reliable_cfg, get_current_position
...
def execute_and_record_trajectory(
    ...
    send_command_reliable_cfg(set_mag, cfg=cfg, cmd_factory=cmd_factory, manager=manager)
    ...
    send_command_reliable_cfg(set_stage_xyr, cfg=cfg, cmd_factory=cmd_factory, manager=manager)
...
def run(cfg: Settings, sem_cfg: FlexSEMSettings):
    ...
    try:
        ...
        for center_x in grid_centers_x:
            for center_y in grid_centers_y:
                ...
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
                                                        "random_walk_2026-03-05", iteration_counter,
                                                        history_filename)
                # all_experiments_data.extend(results)

                traj = generate_major_minor_loop_trajectory("x", base_position, MAJOR_MINOR_LOOP_STEPS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
                                                        "major_minor_x_2026-03-05", iteration_counter,
                                                        history_filename)
                # all_experiments_data.extend(results)

                traj = generate_major_minor_loop_trajectory("y", base_position, MAJOR_MINOR_LOOP_STEPS)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
                                                        "major_minor_y_2026-03-05", iteration_counter,
                                                        history_filename)
                # all_experiments_data.extend(results)

                # circle
                traj = generate_circle_trajectory(base_position, Distance.from_nanometers(3000), 40, ccw=False)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
                                                        "circle_2026-03-05-1", iteration_counter,
                                                        history_filename)
                # all_experiments_data.extend(results)

                traj = generate_circle_trajectory(base_position, Distance.from_nanometers(6000), 100, ccw=True)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
                                                        "circle_2026-03-05-2", iteration_counter,
                                                        history_filename)
                # all_experiments_data.extend(results)

                traj = generate_circle_trajectory(base_position, Distance.from_nanometers(9000), 120, ccw=False)
                results = execute_and_record_trajectory(cfg, cmd_factory, manager, MAGNIFICATION, traj, base_position,
        ...
        # turn off the beam after all experiments are done
        beam_off = cmd_factory.set_beam_state("OFF")
        send_command_reliable_cfg(beam_off, cfg=cfg, cmd_factory=cmd_factory, manager=manager)
