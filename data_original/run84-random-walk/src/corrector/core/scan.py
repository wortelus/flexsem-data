...
def scan(
    ...
    send_command_reliable_cfg(
        set_unfreeze, cfg=cfg, cmd_factory=cmd_factory, manager=manager
    )
    logger.debug(f"Scan unfrozen for {scan_type.name}")

    time.sleep(1)  # Wait for unfreeze to take effect

    # Set scan mode
    set_scan_mode = cmd_factory.set_scan_mode(scan_type)
    send_command_reliable_cfg(
        set_scan_mode, cfg=cfg, cmd_factory=cmd_factory, manager=manager
    )
    ...
    send_command_reliable_cfg(
        set_direct_save, cfg=cfg, cmd_factory=cmd_factory, manager=manager
    )
    ...
    src_img_filepath = cfg.misc.sem_scan_path.with_suffix(
        f".{cfg.misc.sem_scan_image_filetype}"
    )
...
def correction_scan(
    ...
    send_command_reliable_cfg(
        set_magnification, cfg=cfg, cmd_factory=cmd_factory, manager=manager
    )
    ...
    return scan(cfg, cmd_factory, manager, cfg.drift.correction_type, "c")
...
def exposure_scan(
    ...
    mag = step.exposure_mag or cfg.exposure.exposure_mag
