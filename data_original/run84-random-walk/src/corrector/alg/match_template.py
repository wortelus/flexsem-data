...
def match_template(img, template, mask=None):
    ...
    :return: (xy, max_val)
    """
    # @TODO: maybe use TM_CCOEFF_NORMED ?
...
def match_template_with_mask(
    img, template, mask=None, padding: int = 512, border: int = 40
):
    """
    ...
    alignment_dx = found_x - padding + border
    alignment_dy = found_y - padding + border

    # Matching finds the transform which places current content onto the
    # reference.  Public DIC displacement describes current content relative
    # to reference, so the alignment transform is inverted here.
    image_dx = -alignment_dx
    image_dy = -alignment_dy

    return (image_dx, image_dy), max_val


def create_template_matching_mask(
    img_shape, ignore_center_x: int, ignore_center_y: int, bottom_border: int
):
    h, w = img_shape
    ...
    if ignore_center_x > 0 and ignore_center_y > 0:
        cy, cx = h // 2, w // 2
        mask[
            cy - ignore_center_y : cy + ignore_center_y,
            cx - ignore_center_x : cx + ignore_center_x,
        ] = 0.0
