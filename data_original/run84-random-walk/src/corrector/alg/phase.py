...
def phase_correlation(img_a, img_b):
    ...
    :return: (xy, max_val)
    """
    image_displacement, max_val = cv2.phaseCorrelate(img_a, img_b)
    # For arguments (reference, current), OpenCV already returns displacement
    # of current content relative to the reference.  This is not a stage-axis
    # conversion and must not be inverted here.
    return image_displacement, max_val
