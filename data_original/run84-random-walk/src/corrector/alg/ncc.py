...
def find_translation_bruteforce(
    ...
    """Return current-content displacement from reference in image axes.

    ``im1_gray`` is the reference and ``im2_gray`` is the current image.
    """
    ...
    logger.info(
        f"Bruteforce korelace: nejlepší alignment transform {best_shift}, "
        f"důvěra (korelace) {max_corr:.4f}"
    )
    # np.roll searches for the transform which aligns the current image with
    # the reference.  Invert it to return current-content displacement.
