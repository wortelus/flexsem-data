...
def create_windows(df, window_size):
    ...
    try:
        if INVERSE_MODEL:
            inputs = df[['x_actual_abs', 'y_actual_abs']].values
            outputs = df[['x_target_abs', 'y_target_abs']].values
        else:
            inputs = df[['x_target_abs', 'y_target_abs']].values
            outputs = df[['x_actual_abs', 'y_actual_abs']].values
